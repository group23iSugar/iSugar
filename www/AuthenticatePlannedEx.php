...
$selectQuerry = $conn -> prepare("SELECT * FROM useraccount WHERE UserID = ?");
    ...
    $Row = $result->fetch_assoc(); // fetch data   
        ...
        if($flag == "true"){
                $selectQuerry1 = $conn -> prepare("SELECT * FROM plannedexercise WHERE takenInsulinID = ? AND current = ?");
...
$response[] = array("takenInsulinID"=> $takenInsulinID, "typeEx"=> $typeEx, "duration"=> $duration);
