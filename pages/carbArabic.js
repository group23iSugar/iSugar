...
const renderProductList = () => {
  return cho.map((ch) => {
    return <Picker.Item key= {ch.id+''} label={ch.foodArabicName+' - '+ch.unitArabic+'         '} value={ch.id+''} color='black' />
  })
} 
    ...
    <View style={styles.container}>
      ...
      />
          ...
          <TouchableOpacity onPress={() => increment()}>
            <Text style={{color:' black'}}>اضف طبقًا اخر</Text>
            </TouchableOpacity>
  <View style={styles.innerView} >
   
 <View style={styles.innerCotainer2}>
                    <Text style={{fontSize: 15, color: 'grey'}}>كمية الكربوهيدرات</Text>
        ...
        <TouchableOpacity onPress={()=>navigation.navigate('CalcAR', totalCHO)}>
                ...
                >
                    <Text style={styles.textBody}>انتهى</Text>
...
const renderProductList = () => {
  return cho.map((ch) => {
    return <Picker.Item key= {ch.id+''} label={ch.foodArabicName+' - '+ch.unit+'         '} value={ch.id+''} />
  })
} 
