...
const fasting = ({ navigation }) => {
    ...
    try {
      db.transaction(tx => {
        tx.executeSql(
          'SELECT UserID, DOB, weightKG, latest_HP1AC, latest_HP1AC_date, typeOfGlucoseM, glucoseLevel_unit, ketonesMeasure, insulinRegimen, ISF, targetBG_correct, startBG_correct, ISFIntervals, insulinCalcMethod, fromBG, toBG, height,  diagnosis_date FROM patientprofile',
          [],
          (tx, results) => {
            var rows = results.rows;
            for (let i = 0; i < rows.length; i++) {
                console.log(rows.item(i).weightKG + ' weightttttttt');
            ...
            console.log(glucoseMonitorF + ' heyaaaaaaaaaaaa');
          },
        );
      });
    } catch (error) {
              ...
              tx.executeSql(
                ...
                (tx, results) => {
                  ...
                  for (let i = 0; i < rows.length; i++) {
                    var UID = rows.item(i).UserID;
                    console.log(UID);
                    console.log(rows.item(i).insulinRegimen + ' lplplplpllplpl');
                  }
                },
              );
            });
