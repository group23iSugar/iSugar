...
const register = async () => {
     if ( data.fName.trim().length>2 && data.lName.trim().length>2 && data.isValidEmail == true){
   ...
   console.log(error);
       ...
       if (data.isValidPassword == true){
        ...
        alert('على كلمة المرور أن تحوي على الأقل حرف واحد كبير ورمز مميز، وأن تكون أطول من 8 ');
       }
...
if (response[0].Message == "false"){
  alert('هذا البريد الإلكتروني موجود من قبل');
} else {
  getOnlineInfo(); 
}
})
