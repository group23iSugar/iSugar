  ...
  const icr = ({ navigation, route }) =>{
    ...
    //-------------------------------
     ...
     if(AccType == 'Patient Account'){
      var {insulinSF, intervalISF, tBG, sBG, moni, unit, ketoneM, bgF, bgT, cen, MRN, DOD, DOB, DOH, Weight, height, LH, cName, cCity, insulinR } = route.params;
      
     } else {
      var {insulinSF, intervalISF, tBG, sBG, DOB, DOH, Weight, LH, moni, unit, ketoneM, bgF, bgT, insulinR} = route.params;
     }
    useEffect(() => {
      MethodForCalc('ICR');
        }, []);
      ...
      try {
         db.transaction( (tx) => {
             tx.executeSql(
              ...
              +'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                [uID, DOB, Weight, LH, DOH, moni, unit, ketoneM, insulinR, insulinSF, tBG, sBG, intervalISF, data.caluMethod, bgF, bgT, height, cen, DOD, cName, cCity ]
            );
    ...
    try {
      db.transaction( (tx) => {
          tx.executeSql(
           ...
           +'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
             [uID, DOB, Weight, LH, DOH, moni, unit, ketoneM, insulinR, insulinSF, tBG, sBG, intervalISF, data.caluMethod, bgF, bgT]
         );
...
{data.caluMethod == 'ICR'? (<TouchableOpacity onPress={()=>patientProfile()}>
                <LinearGradient
