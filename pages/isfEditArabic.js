...
import { ActivityIndicator, Colors } from 'react-native-paper';
  ...
  const isfEditAR = ({ navigation, route }) =>{
    useEffect(() => {
...
const handleAllDayUpdate = ()=> {
  if (isfP>999 || isfP<=0  || sBGP>999 || sBGP <= 0 || tBGP >999 || tBGP <=0){
    alert('من فضلك قم بإدخال رقم صحيح');
    return;
  } else {
    if (AccType == 'Patient Account'){
      try {
  ...
  const localISF = () => {
    ...
    return (
      ...
      <View style={styles.footer}>
       <Text style={styles.title}>تعديل معامل الانسولين {'\n'}</Text>
...
<View style={styles.actionB}>
              <Text style={styles.text_footer}>وقت بداية التصحيح{'\n'}correction</Text>
             <Text style={styles.text_footer}>{isfInterval=='0'? 'كل الوقت' : 'اوقات محددة'}</Text>
...
: null}
  {isfInterval=='1' ? hours.length > 0 ? ( (<View style={{alignItems:'center'}}>
          ...
          />
          <TouchableOpacity 
          onPress={()=>addISF()}><Text style={{fontSize: 15, color: '#05375a', textDecorationLine: 'underline' }} >اضف مدة</Text></TouchableOpacity>
...
const changeV = (val, id, type, i) => { 
  ...
  const localISF = () => {
    ...
    //---------------------------
    const onlineIntervalDB = () => {
      var InsertAPIURL = "http://192.168.12.1/isugar/ISFIntervals.php"; 
