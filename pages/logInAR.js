...
const logIn = ({navigation}) => {
  // var isAuthenticated = "";
   ...
   const logAlert = () => {
     Alert.alert(
       ...
       '',
       //body
       'الإيميل أو الرقم السري غير صحيح',
       [
         {
           text: 'حسنًا',
           onPress: () => {
         ...
         db.transaction( (tx) => {
           tx.executeSql(
            'INSERT INTO isfInterval (UserID, fromTime, toTime, ISF, targetBG_correct, startBG_correct) VALUES (?,?,?,?,?,?)',
              [1, fromTimeISF[i], toTimeISF[i], ISF_[i], targetBGISF[i], startBGISF[i]]
          );
         ...
         db.transaction( (tx) => {
           tx.executeSql(
            'INSERT INTO icrInterval (UserID, fromTime, toTime, ICR) VALUES (?,?,?,?)',
              [1, fromTime1[i], timeTo1[i], ICR1[i] ]
          );
 ...
 //To find sliding scale id from local database 
         ...
         db.transaction( (tx) => {
           tx.executeSql(
            'INSERT INTO takenInsulinDose (UserID, BG_Level, ReasonForInsulin, CHO, insulinDose, Dose_time_hours, Dose_time_minutes, Dose_Date_Month, Dose_Date_Day, Dose_Date_Year) VALUES (?,?,?,?,?,?,?,?,?,?)',
              [1, BGLevel, reason, CHO, insulinDose, doseHour, doseMin, doseMonth, doseDay, doseYear]
   ...
   .then((response)=>{
     ...
     console.log(response[0].userID + ' user ID lololololoololo');
         ...
         <View style={styles.buttonV}>
           <TouchableOpacity onPress={() => logI()}>
             <LinearGradient
               ...
               <Text style={styles.titleB}>تسجيل الدخول</Text>
             </LinearGradient>
           </TouchableOpacity>
         </View>
