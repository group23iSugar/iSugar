          ...
          (tx, results) => {
            ...
            for (let i = 0; i < rows.length; i++) {
              ...
              if (UID == 1) {
                ...
                console.log(DODF + ' heyaaaaaaaaaaaa');
        ...
        try {
            db.transaction( (tx) => {
                tx.executeSql(
                 ...
                 + 'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                   [1, DOBirthF, weightKGF, latestHB1AC_F, DOLatestHB1ACF, glucoseMonitorF, glucoseUnitF, ketonesMeasureF, insulinRegF, InsulinSFF, bgTargetF, bgStartF, intervalISFF, insulinCalcMethodF, fromBGF, toBGF, heightCMF, DiabetescenterF, DODF, centNameF, centCityF ]
               );
           })
           try {
            db.transaction(tx => {
              ...
              try {
                db.transaction( (tx) => {
                    tx.executeSql(
                     ...
                     + 'VALUES (?,?,?,?)',
                       [1, ICRFasting[i].from, ICRFasting[i].to, ICRFasting[i].icr ]
                   );
               })
               try {
                db.transaction(tx => {
              ...
              try {
                ...
                db.transaction( (tx) => {
                    tx.executeSql(
                     ...
                     + 'VALUES (?,?,?)',
                       [1, SSFasting[i].FromTime, SSFasting[i].toTime]
                   );
            ...
            try {
              db.transaction( (tx) => {
                  tx.executeSql(
                   ...
                   + 'VALUES (?,?,?,?,?,?)',
                     [1, ISFFasting[i].from, ISFFasting[i].to, ISFFasting[i].ISF_, ISFFasting[i].target, ISFFasting[i].start ]
                 );
             })
             try {
              db.transaction(tx => {
        ...
        try {
          db.transaction( (tx) => {
              tx.executeSql(
               ...
               + 'VALUES (?,?,?)',
                 [1, plannedExerciseFasting[i].type, plannedExerciseFasting[i].duration ]
             );
         })
        ...
        try {
          db.transaction( (tx) => {
              tx.executeSql(
               ...
               + 'VALUES (?,?,?)',
                 [1, previousExerciseFasting[i].type, previousExerciseFasting[i].duration, previousExerciseFasting[i].Time ]
             );
         })
            ...
            try {
                db.transaction( (tx) => {
                    tx.executeSql(
                     ...
                     + 'VALUES (?,?,?,?)',
                       [1, new Date(), moment(new Date()).format('h:mm a'), 'false']
                   );
               })
               try {
                db.transaction(tx => {
                ...
                try {
                  db.transaction( (tx) => {
                      tx.executeSql(
                       ...
                       + 'VALUES (?,?,?)',
                         [1, penFating[i].insulinType, penFating[i].halfORfull]
                     );
                 })
                 try {
                  db.transaction(tx => {
       ...
       <Text
         ...
         <View style={styles.body}>
          ...
          {yesFlag || noFlag ? null : ( <View>
            <Text style={styles.textHeader}>هل ترغب برؤية هذه الرسالة يوميا؟</Text>
           ...
           {typeFlag ? (<View>
            <View style={styles.innerCotainer}>
          <Text style={styles.textAlert}>التوصيات لصيام رمضان غير مدعومة في التطبيق لمن يستخدم انسولين {nameG}. الرجاء التواصل مع فريق السكر للحصول على التوصيات.</Text>

          </View>
           </View>) : (
             <View />
           )}
        <TouchableOpacity onPress={()=>{navigation.navigate('fastingProfileAR'); }}>
            <LinearGradient style={styles.outer}
            colors={['#a8dadc', '#457b9d']}>
            <View style={styles.inner}>
            <Text style={styles.textBody}> إعدادات الانسولين لصيام شهر رمضان </Text>
            </View>

            </LinearGradient>
            </TouchableOpacity>
     {/* //=================// */}
     <TouchableOpacity onPress={()=>{navigation.navigate('calcFastingAR'); }}>
            <LinearGradient style={styles.outer}
            colors={['#a8dadc', '#457b9d']}>
            <View style={styles.inner}>
            <Text style={styles.textBody}>حاسبة جرعات الانسولين خلال صيام رمضان</Text>
            </View>
            ...
            {/* //=================// */}
            <TouchableOpacity onPress={()=>{navigation.navigate('calcRecommendAR'); }}>
            <LinearGradient style={styles.outer}
            colors={['#a8dadc', '#457b9d']}>
            <View style={styles.inner}>
            <Text style={styles.textBody}>التوصيات لتعديل جرعة الانسولين القاعدي (طويل المفعول) أثناء صيام رمضان</Text>
            </View>

            </LinearGradient>
            </TouchableOpacity>
            {/* //======= ==========// */}
            <TouchableOpacity onPress={()=>{navigation.navigate('guidlinesAR'); }}>
            <LinearGradient style={styles.outer}
            colors={[ '#a8dadc', '#457b9d']}>
            <View style={styles.inner}>
            <Text style={styles.textBody}>توصيات عامة لمرض السكري وصيام رمضان</Text>
            </View>
            </LinearGradient>
            </TouchableOpacity>
            {/* //=================// */}
