  ...
  const navInstr = async() => {
    ...
    if (hBGlevel <= 70) {
      ...
      if (recheckCounter == 3) {
        ...
        navigation.navigate('isConPopAR');
      }} else if (hBGlevel > 70){ //No more Hypo or no Hypo at all

        if (recheckCounter > 0){
          recheckCounter = 0;
          navigation.navigate('reNoHypoAR');
        } else if (recheckCounter == 0){
          navigation.navigate('normBGfirstAR');
        }
      }
    //  else {
  ...
  return (
    <LinearGradient colors={['#AABED8', '#fff']} style={styles.container}>
      ...
      <ScrollView style={styles.contView}>
        ...
        <Text style={styles.inpTxt}>ادخل مستوى سكر الدم</Text>
        ...
        <TouchableOpacity
          ...
          onPress={navInstr}>
          <Text style={{fontSize: 18, textAlign: 'center'}}>اذهب</Text>
        </TouchableOpacity>
      </ScrollView>
    </LinearGradient>
  );
